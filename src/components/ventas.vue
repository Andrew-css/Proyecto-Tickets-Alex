  ...
  <div>
    ...
    <div v-if="mostrarFormulario" class="formulario">
      ...
      <q-select filled v-model="cliente" clearable use-input hide-selected fill-input input-debounce="0"
                label="Digite cédula cliente"
                :options="clientes.map(c => ({ label: c.cedula, value: c }))"
      ...
      <button type="button" @click="vender">Vender</button>
    </div>
    ...
    <div class="modal-content">
      <form class="form">
        <div class="cerrar">
          ...
          <button type="button" data-bs-dismiss="modal" @click="cerrarAgregarCliente()" class="row justify-center items-center" id="botoncerrar">❌</button>
        </div>
...
watch(ruta, (nuevaRuta) => {
  ...
  console.log("Hola soy datos ruta: ",nuevaRuta)
  console.log("Hola soy datos ruta: ",ruta)
...
watch(cliente, (nuevoCliente) => {
  console.log("Hola soy datos cliente: ",nuevoCliente)
  console.log("Hola soy datos cliente especifico: ",cliente)
...
console.log("Hola soy vendedor: ", useTiquete.tokenvendedor)
  ...
  /* const data = {
    ...
    try {
      ...
      if (useTiquete.estatus === 200) {
        ...
        mensaje.value = 'Tiquete añadido correctamente (presione ❌ para cerrar)';
        setTimeout(() => {
          cliente.value = '';
          cedula.value = '';
          email.value = '';
...
async function obtenerClientes() {
  try {
    await useCliente.obtenerClientes(); // Asumiendo que tienes un método en useConductor para obtener conductores
...
.asiento-seleccionado {
  background-color: #4caf50;  /* Cambia el color de fondo al asiento seleccionado */
  border: 2px solid #388e3c;  /* Cambia el borde al asiento seleccionado */
  color: #fff;  /* Cambia el color del texto al asiento seleccionado si es necesario */
}
...
.formulario {
  ...
  padding: 20px;  /* Padding para separar del borde */
}
